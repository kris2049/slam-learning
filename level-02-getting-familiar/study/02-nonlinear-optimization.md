...
代入代价函数：
$$\|r + J\Delta\theta\|^2 = (r + J\Delta\theta)^{\mathsf{T}}(r + J\Delta\theta)$$
$$= r^{\mathsf{T}}r + 2r^{\mathsf{T}}J\Delta\theta + \Delta\theta^{\mathsf{T}}J^{\mathsf{T}}J\Delta\theta$$

对 $\Delta\theta$ 求导并令其为零：
$$\frac{\partial}{\partial \Delta\theta} = 2J^{\mathsf{T}}r + 2J^{\mathsf{T}}J\Delta\theta = 0$$

$$\Rightarrow J^{\mathsf{T}}J \Delta\theta = -J^{\mathsf{T}}r$$

$$\Rightarrow H \Delta\theta = -g$$

其中 $H = J^{\mathsf{T}}J$ 是近似的**Hessian矩阵**，$g = J^{\mathsf{T}}r$ 是梯度。
...
- $T_1 + T_2$ 无意义（$T_1 + T_2 \notin SE(3)$）
- 直接用矩阵做优化不保证 $R^{\mathsf{T}}R = I, \det(R)=1$
...
$$H = \begin{bmatrix} B & E \\ E^{\mathsf{T}} & C \end{bmatrix}$$
...
其中 $S = B - E C^{-1} E^{\mathsf{T}}$（**舒尔补**）。

1. 求解小系统 $S \Delta\mathbf{x}_c = ...$ → 相机更新
2. 回代 $\Delta\mathbf{x}_p = C^{-1}(\mathbf{b}_p - E^{\mathsf{T}}\Delta\mathbf{x}_c)$ → 路标更新
...
$$H_{marg} = H_{bb} - H_{ab}^{\mathsf{T}} H_{aa}^{-1} H_{ab}$$
